...
const FeaturedScholarships = ({ scholarships: initialScholarships = [] }: FeaturedScholarshipsProps) => {
  // If we have server-side data, use it directly
  // Otherwise, fetch data client-side
  const { data: fetchedScholarships = [], isLoading } = useQuery<Scholarship[]>({
    queryKey: ['/api/scholarships'],
    // Skip the query if we already have data from server
    enabled: initialScholarships.length === 0,
    initialData: initialScholarships.length > 0 ? initialScholarships : undefined,
  });
  
  // Use either server-provided or client-fetched data
  const scholarships = initialScholarships.length > 0 ? initialScholarships : fetchedScholarships;
            ...
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-card rounded-2xl shadow-md overflow-hidden border border-border">
                <div className="p-6">
                  ...
                  <div className="h-8 w-3/4 mb-3 bg-gray-200 animate-pulse" />
                  <div className="h-4 w-full mb-1 bg-gray-200 animate-pulse" />
                  <div className="h-4 w-full mb-1 bg-gray-200 animate-pulse" />
                  <div className="h-4 w-2/3 mb-5 bg-gray-200 animate-pulse" />
