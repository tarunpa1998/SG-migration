...
// Helper function to get auth token safely on client side
...
export const authApi = {
  ...
  register: async (userData: RegisterData): Promise<AuthResponse> => {
    ...
    try {
      // Use the API endpoint to proxy to Express backend
      const response = await api.post<AuthResponse>('/api/direct-api/auth/register', userData);
  ...
  login: async (credentials: LoginData): Promise<AuthResponse> => {
    ...
    try {
      // Use the API endpoint to proxy to Express backend
      const response = await api.post<AuthResponse>('/api/direct-api/auth/login', credentials);
  ...
  googleLogin: async (token: string): Promise<AuthResponse> => {
    ...
    try {
      // Use the API endpoint to proxy to Express backend
      const response = await api.post<AuthResponse>('/api/direct-api/auth/google', { token });
  ...
  getCurrentUser: async (): Promise<AuthUser> => {
    try {
      ...
      // Use the API endpoint to proxy to Express backend
      const response = await api.get<AuthUser>('/api/direct-api/auth/user');
  ...
  saveArticle: async (articleId: string): Promise<{ message: string, savedArticles: string[] }> => {
    const response = await api.post(`/api/direct-api/user/save-article/${articleId}`);
    return response.data;
  },

  // Unsave article
  unsaveArticle: async (articleId: string): Promise<{ message: string, savedArticles: string[] }> => {
    const response = await api.delete(`/api/direct-api/user/unsave-article/${articleId}`);
    return response.data;
  },

  // Save scholarship
  saveScholarship: async (scholarshipId: string): Promise<{ message: string, savedScholarships: string[] }> => {
    const response = await api.post(`/api/direct-api/user/save-scholarship/${scholarshipId}`);
    return response.data;
  },

  // Unsave scholarship
  unsaveScholarship: async (scholarshipId: string): Promise<{ message: string, savedScholarships: string[] }> => {
    const response = await api.delete(`/api/direct-api/user/unsave-scholarship/${scholarshipId}`);
    return response.data;
  },

  // Like article
  likeArticle: async (articleId: string): Promise<{ message: string, likes: number }> => {
    const response = await api.post(`/api/direct-api/likes/article/${articleId}`);
    return response.data;
  },

  // Unlike article
  unlikeArticle: async (articleId: string): Promise<{ message: string, likes: number }> => {
    const response = await api.delete(`/api/direct-api/likes/article/${articleId}`);
    return response.data;
  },

  // Get article like count
  getArticleLikes: async (articleId: string): Promise<{ likes: number }> => {
    const response = await api.get(`/api/direct-api/likes/article/${articleId}/count`);
    return response.data;
  },

  // Check if user liked article
  hasLikedArticle: async (articleId: string): Promise<{ hasLiked: boolean }> => {
    const response = await api.get(`/api/direct-api/likes/article/${articleId}/status`);
    return response.data;
  },

  // Post comment on article
  postComment: async (articleId: string, content: string): Promise<any> => {
    const response = await api.post(`/api/direct-api/comments/article/${articleId}`, { content });
    return response.data;
  },

  // Get comments for article
  getArticleComments: async (articleId: string): Promise<any[]> => {
    const response = await api.get(`/api/direct-api/comments/article/${articleId}`);
    return response.data;
  },

  // Update user profile
  updateProfile: async (data: { fullName?: string, profileImage?: string }): Promise<AuthUser> => {
    const response = await api.put('/api/direct-api/user/profile', data);
    return response.data;
  },

  // Get user's comments
  getUserComments: async (): Promise<any[]> => {
    const response = await api.get('/api/direct-api/user/comments');
    return response.data;
  },
